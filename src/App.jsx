...
function parseIntent(msg) {
  ...
  if (numMatch) return { type:"order_detail", numero: numMatch[1] };
  ...
  switch(intent.type) {
    ...
    case "receivable": {
      ...
      const vencidas = contas.filter(c=>{
        ...
        const [d,mo,y]=v.split("/"); return new Date(y,mo-1,d)<nowSP();
      }).length;
    ...
    case "dashboard": {
      const [pR, rR, pgR] = await Promise.all([
...
function ResultBlock({result}) {
      ...
      <div style={{marginTop:10}}>
        ...
        {contas.length===0
          ...
          : contas.slice(0,30).map((c,i)=>{
              const ct=c?.conta??c;
              const isVenc=(()=>{ const v=ct.vencimento; if(!v)return false; const [d,mo,y]=v.split("/"); return new Date(y,mo-1,d)<nowSP(); })();
              return (
  ...
  if (result.type === "dashboard") {
    ...
    const recVenc=receber.filter(c=>{ const v=c?.conta?.vencimento; if(!v)return false; const [d,mo,y]=v.split("/"); return new Date(y,mo-1,d)<nowSP(); }).length;
    return (
      <div style={{marginTop:10}}>
